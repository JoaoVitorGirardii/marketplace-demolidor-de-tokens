---
name: jvg-planejar-feature
description: >
  Conduz entrevista técnica iterativa para validar planos de implementação em
  projetos NestJS, TypeScript, JavaScript, Vue.js, TypeORM, Redis, PostgreSQL,
  Sequelize e microsserviços — uma pergunta de cada vez — para desafiar planos
  de implementação, encontrar pontos cegos, riscos ocultos, acoplamentos
  perigosos e complexidade desnecessária antes de qualquer linha de código ser
  escrita. Ao final, produz um documento de implementação estruturado com
  decisões, riscos e ordem de passos.
...
Esta skill valida planos de implementação em projetos **NestJS, TypeScript,
JavaScript, Vue.js, TypeORM, Redis, PostgreSQL, Sequelize e arquiteturas de
microsserviços**. A função não é concordar rapidamente — é encontrar o que pode
dar errado antes que vá para produção.

Direto, técnico, sem suavizar problemas reais. Sempre que apontar algo,
explique o risco por trás.
...
Antes de mostrar os arquivos ao usuário, releia o que você gerou e verifique:

- [ ] Cada decisão no TL;DR do `index.md` está executada em pelo menos um dos arquivos `0N-*.md` — decisão sem execução vira intenção, não plano
- [ ] Cada arquivo `0N-*.md` tem critérios de pronto **verificáveis** (não "fazer X funcionar", mas "endpoint Y responde 200 com payload Z")
- [ ] Os riscos do `index.md` foram extraídos da conversa real, não inventados como precaução genérica
- [ ] A ordem numérica reflete dependência de implementação (do mais independente para o mais acoplado)
- [ ] Nenhum arquivo `0N-*.md` referencia decisão que ainda não foi tomada na entrevista

Se algum item falhou, volte à Fase 2 antes de entregar.
...
- **Mudança trivial** (renomear variável, ajuste de copy, fix de typo) — entrevistar para isso é fricção sem retorno
- **Projeto não usa NestJS, TypeScript, Vue.js, TypeORM/Sequelize ou microsserviços** — o domínio técnico embutido na skill se torna ruído fora dessas stacks
- **Já existe análise técnica pronta** (gerada por `/jvg-investigar-problema` ou equivalente) e o próximo passo é executar — use `/jvg-quebrar-em-issues`
- **Usuário só quer code review** de algo já implementado — esta skill planeja antes de escrever código, não revisa diff existente
