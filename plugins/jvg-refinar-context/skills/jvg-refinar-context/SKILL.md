---
name: jvg-refinar-context
description: >
  Atualiza e aprimora cirurgicamente um CONTEXT.md existente (gerado pela skill
  jvg-gerar-context). Faz perguntas precisas e pertinentes ao usuário para entender
...
Se não houver `CONTEXT.md`, informe o usuário e sugira rodar `/jvg-gerar-context`
...
Com base no que você leu, conduza uma conversa para entender exatamente o que
deve mudar. Não faça perguntas genéricas — cada pergunta deve referenciar algo
concreto do documento ou da solicitação.

**A regra mais importante:**

> Faça **uma única pergunta** por mensagem. Depois de fazê-la, **pare e aguarde**.
> Nunca empilhe perguntas. Cada resposta pode tornar a próxima pergunta irrelevante
> ou revelar algo que muda completamente o escopo — só você sabe quando parou de
> ter dúvidas.

### Perguntas sempre relevantes (use uma por vez, adapte ao contexto):
...
Continue perguntando uma coisa por vez até ter clareza total sobre o escopo.
Nunca avance para a edição com dúvidas abertas.
...
**Uma pergunta por turno, sem exceção.** Faça a pergunta, pare, espere a resposta.
Nunca empilhe perguntas numa mesma mensagem — o usuário responde só parte e você
perde informação crítica sobre o escopo.
...
- **Não existe `CONTEXT.md`** no projeto — use `/jvg-gerar-context` primeiro para gerar a base
- **A mudança é uma reescrita completa** do documento (mais de 60% das seções afetadas) — vale rodar `/jvg-gerar-context` de novo em vez de refinar
- **O usuário quer melhorar o código que está documentado** — esta skill toca o documento, não o código
- **Existem vários `CONTEXT-*.md` espalhados** e o usuário não disse qual atualizar — pergunte antes de presumir; a skill opera num único documento por execução
