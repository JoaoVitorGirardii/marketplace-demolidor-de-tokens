...
description: Investiga dores de usuário antes de qualquer decisão. Conduz uma conversa estruturada para entender o problema, contexto, impacto real e riscos para outros usuários, analisa o código do projeto em segundo plano para enriquecer as perguntas, verifica se já existe documentação similar, e só então propõe a criação de um arquivo .md com o diagnóstico — sempre com aprovação do usuário. Use quando alguém relatar uma dor, fricção ou problema no sistema ("o sistema está lento", "precisa de uma melhoria", "isso me incomoda", "temos um bug em X", "seria bom se o sistema fizesse Y") — investigue a fundo antes de sugerir qualquer solução.
---
...
## Fase 3 — Consolidação e Aprovação
...
> _Contexto para o time de desenvolvimento — não discutido diretamente com o usuário_
...
> "Esse diagnóstico captura bem o problema? Quer ajustar alguma coisa antes de eu criar o arquivo?"

Itere até que o usuário aprove o conteúdo.

---

## Fase 3b — Auto-revisão antes de criar o arquivo

Antes de criar o arquivo, releia o documento e verifique:
...
- [ ] As labels sugeridas batem com o tipo de problema — não marcar `bug` se é melhoria de UX, não marcar `enhancement` se é regressão
...
## Fase 4 — Criação do Arquivo

Somente após aprovação explícita, sugira o nome do arquivo com base no título do diagnóstico (kebab-case, sem acentos, máximo 50 caracteres) e crie o arquivo no diretório atual ou no diretório que o usuário indicar:

```
<nome-do-problema>.md
```

Exemplo: para "Lentidão ao exportar relatórios" → `lentidao-exportar-relatorios.md`

Crie o arquivo com o conteúdo completo do documento gerado e confirme ao usuário o caminho onde foi salvo.
...
- **Confirme antes de criar** — o usuário aprova o documento antes de qualquer arquivo ser criado
- **Seja um filtro** — se o problema não for real, já estiver documentado, ou for falta de treinamento, diga com clareza e encerre com orientação útil
...
- **A "dor" é falta de treinamento** ou desconhecimento do sistema — esta skill documenta problemas, não tira dúvida; oriente o usuário antes de criar qualquer arquivo
